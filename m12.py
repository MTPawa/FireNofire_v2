...
If Windows gives DataLoader multiprocessing issues, use:
...
def train_model(args: argparse.Namespace) -> None:
    ...
    final_results = {"final_reporting_model": "best validation accuracy model", "model": model_name, "model_display": display, "best_epochs": best_epoch, "best_accuracies": best, "final_evaluation_using_best_val_model": final_eval, "class_to_idx": class_to_idx, "idx_to_class": idx_to_class, "run_collection_dir": str(runs_root), "run_name": run_name, "weights_dir": str(weights_dir), "results_dir": str(results_dir), "output_organization": "root/weights/Run_XX and root/results/Run_XX", "best_weight_paths": {m: {"checkpoint": str(weights_dir / f"best_{m}_model.pth"), "state_dict": str(weights_dir / f"best_{m}_model_state_dict_only.pth")} for m in ["val", "train", "test", "test_e"]}, "training_history_path": str(results_dir / "training_history.json"), "training_log_csv_path": str(csv_path), "config_path": str(results_dir / "config.json"), "dataset_balance_report_path": str(results_dir / "dataset_balance_report.json"), "image_resize": {"height": img_h, "width": img_w, "size_tag": size_tag, "source": size_source}, "device_info": info}
...
def run_repeated_training(args: argparse.Namespace) -> None:
    """
    Run full training args.repeat times.

    Each repeat:
    - gets a separate Run_XX folder
    - uses seed = base_seed + repeat_index
    - saves its own weights/results
    - is included in repeat_summary.csv/json
    """
    ...
    print("\n" + "=" * 80)
    print(f"Running repeated training: {repeat_count} repeats")
    print(f"Base seed: {base_seed}")
    print("=" * 80 + "\n")

    for repeat_idx in range(1, repeat_count + 1):
        repeat_args = copy.deepcopy(args)
        repeat_args.seed = base_seed + repeat_idx - 1

        print("\n" + "#" * 80)
        print(f"STARTING REPEAT {repeat_idx}/{repeat_count}")
        print(f"Seed for this repeat: {repeat_args.seed}")
        print("#" * 80 + "\n")
        ...
        print("\n" + "#" * 80)
        print(f"FINISHED REPEAT {repeat_idx}/{repeat_count}")
        print("#" * 80 + "\n")

    # Save summary inside the common results folder:
    # Runs/<Model>_<Size>_01/results/repeat_summary.csv/json
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset_base", type=str, default="Dataset", help="Base dataset folder used with --dataset_size. Default: Dataset")
    ...
    parser.add_argument("--data_root", type=str, default=None, help="Direct dataset path containing train/val/test/test_e, e.g. Dataset/128x128. Recommended.")
    ...
    parser.add_argument("--num_workers", type=int, default=4, help="DataLoader workers. Use 0 if Windows has multiprocessing issues.")

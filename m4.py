...
def train_model(args: argparse.Namespace) -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"{'BEST' if is_best else f'No improve: {epochs_without_improvement}'}"
        )
    ...
    # Load the best model and do final evaluation on all splits.
    best_checkpoint = torch.load(checkpoints_dir / "best_model.pth", map_location=device)
    ...
    final_results = {
        ...
        "note": "test and test_e were evaluated only once after training using the best validation-accuracy model.",
        "class_to_idx": class_to_idx,
        "idx_to_class": idx_to_class,
        "run_dir": str(run_dir),
        "best_model_path": str(checkpoints_dir / "best_model.pth"),
    ...
    print(f"Best weights      : {checkpoints_dir / 'best_model.pth'}")
...
def plot_training_curves(history: Dict[str, Any], plots_dir: Path) -> None:
    ...
    During training this script only logs:
    - train
    - val

    test and test_e are evaluated only once after training with the best model.
    """
